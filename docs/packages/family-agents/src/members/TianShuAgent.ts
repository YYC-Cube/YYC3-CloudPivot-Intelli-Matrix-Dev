...
export class TianShuAgent extends FamilyBaseAgent {
  ...
  protected setupFamilyCommandHandlers(): void {
    this.registerCommandHandler('route', async (params) => this.route(params.intent));
    this.registerCommandHandler('decompose', async (params) => this.decompose(params.intent));
  }
