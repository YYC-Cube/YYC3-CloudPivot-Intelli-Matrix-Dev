...
/**
 * file: detector.ts
 * description: 语言检测器 — 系统语言识别、标准化与中文判断
 * author: YanYuCloudCube Team <admin@0379.email>
 * version: v2.0.0
 * created: 2026-03-21
 * updated: 2026-06-09
 * status: active
 * tags: [util],[i18n],[locale],[detection]
 *
 * brief: 提供系统语言检测、locale 别名标准化及中文字符集判断
 *
 * details:
 * - 支持浏览器 navigator.language 检测与 Node.js 环境变量回退
 * - 42 个常见 locale 别名的标准化映射（zh → zh-CN, en-us → en 等）
 * - 中文字符集正则检测（CJK 统一表意文字区间）
 * - normalizeLocale() 大小写规范化与回退链
 *
 * dependencies: types.js
 * exports: detectSystemLocale, normalizeLocale, isChineseLocale
 * notes: 浏览器端依赖 window.navigator，Node 端依赖 process.env.LANG
 */
