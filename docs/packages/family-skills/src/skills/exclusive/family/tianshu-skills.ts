...
export const crisisResponseSkill = defineSkill(
  ...
  async (params) => {
    ...
    const RESPONSE_ROLES: Record<string, string[]> = {
      security: ['guardian', 'tianshu'],
      performance: ['thinker', 'prophet'],
      data_loss: ['guardian', 'grandmaster'],
      service_down: ['tianshu', 'grandmaster'],
      code_bug: ['grandmaster', 'guardian'],
    ...
    return {
      ...
      affectedSystems,
      status: 'active',
      escalation: score >= 0.8 ? 'escalate-to-tianshu' : 'standard-response',
    };
  },
  (params) => {
    const errors: string[] = [];
