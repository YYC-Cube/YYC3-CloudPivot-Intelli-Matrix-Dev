...
import { useInjectedHooks } from "./HooksInjectionContext";
import { useModelProvider as useModelProviderStub } from "./stubs/useModelProvider";
import { useSettingsStore as useSettingsStoreStub } from "./stubs/useSettingsStore";
...
export function useAIConfig(): UseAIConfigReturn {
  // 运行时注入：如果消费者通过 HooksProvider 提供了真实 hooks，优先使用
  const injected = useInjectedHooks();
  const useModelProvider = injected?.useModelProvider ?? useModelProviderStub;
  const useSettingsStore = injected?.useSettingsStore ?? useSettingsStoreStub;
