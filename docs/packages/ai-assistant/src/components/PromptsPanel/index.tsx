...
export function PromptsPanel({ activePrompt, onSelect, onCustomChange }: PromptsPanelProps) {
  ...
  return (
    <div className="min-h-0">
      <h4 className="text-[#e0f0ff] mb-3 text-sm font-medium">{t("prompts.presetTitle")}</h4>
      ...
      <h4 className="text-[#e0f0ff] mb-2 text-sm font-medium">{t("prompts.customTitle")}</h4>
      <textarea
        value={activePrompt}
      ...
      <p className="text-[rgba(0,212,255,0.25)] mt-1 text-xs">
        {t("prompts.charCount", { count: activePrompt.length })}
      </p>
    </div>
  );
}
