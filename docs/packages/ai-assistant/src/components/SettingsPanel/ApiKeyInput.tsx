 ...
 * @description API Key 输入组件（接入 i18n）
      ...
      <div className="relative">
        ...
        >
          <span className="text-[rgba(0,212,255,0.4)] text-xs">
            {showValue ? t("settings.hide") : t("settings.show")}
          </span>
        </button>
      </div>
      <p className="text-[rgba(0,212,255,0.25)] mt-1 text-xs">
        {value ? t("settings.apiKeyConfigured") : t("settings.apiKeyMissing")}
        {" · "}
        {t("settings.apiKeyStorageNote")}
      </p>
    </div>
  );
}
