...
export class GraceAgent extends FamilyBaseAgent {
  ...
  protected setupCommandHandlers(): void {
    this.registerCommandHandler('create', async (params) => this.create(params as { prompt: string; type?: string; style?: string }));
    this.registerCommandHandler('design', async (params) => this.designSuggest(params as { aspect: string; context?: string }));
  }
