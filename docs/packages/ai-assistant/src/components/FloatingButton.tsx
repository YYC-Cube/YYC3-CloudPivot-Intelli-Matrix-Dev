 ...
 * @description AI 助手浮动触发按钮（接入 i18n）
...
export function FloatingButton({ onClick, isMobile = false, tooltip }: FloatingButtonProps) {
  const { t } = useI18n();
  const tooltipText = tooltip ?? t("brand.tooltip");
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-full right-0 mb-2 px-3 py-1.5 rounded-lg bg-[rgba(8,25,55,0.95)] border border-[rgba(0,180,255,0.2)] opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
        <span className="text-[#00d4ff] text-xs">{tooltipText}</span>
      </div>
    </button>
  );
}
